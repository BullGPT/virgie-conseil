        ...
        >
          ...
          >
            <ul className="flex flex-col gap-1 py-2 lg:flex-row lg:items-center lg:justify-center lg:gap-1 lg:py-0">
              {mainNav.map((link) => {
                // Une ancre s'allume selon la section à l'écran ; un lien de
                // page classique, selon la route courante.
                const anchor = idFromHref(link.href);
                const active = anchor
                  ? pathname === "/" && activeId === anchor
                  : pathname === link.href;
                return (
                  <li key={link.href}>
                    <Link
                      ...
                      aria-current={active ? (anchor ? "location" : "page") : undefined}
                      className={cn(
